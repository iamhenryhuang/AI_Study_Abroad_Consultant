...
async def get_page_content(page, url: str, school_domain: str) -> str | None:
    try:
        ...
        # --- 加入黑名單過濾的文字提取邏輯 ---
        # 注意：先在 Python 端把 CONTENT_BLACKLIST 轉成 JSON 字串，避免 JS template 字串出問題
        blacklisted_json = json.dumps(CONTENT_BLACKLIST, ensure_ascii=False)

        content = await page.evaluate(
            f"""
            () => {{
                const clones = document.body.cloneNode(true);
                const blacklistedTags = {blacklisted_json};

                // 排除 class 或 id 包含黑名單關鍵字的元素 (例如 bjkwfe-nav)
                blacklistedTags.forEach(word => {{
                    const elements = clones.querySelectorAll(
                        `[class*="${{word}}" i], [id*="${{word}}" i]`
                    );
...
if __name__ == "__main__":
    target_schools = [
        ...
        #"https://masters.cs.uchicago.edu/mpcs-admissions/" #芝加哥會擋
