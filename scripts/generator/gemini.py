...
def generate_admission_analysis(query, context_docs, profile=None, school_stats=None):
    """
    將使用者所有資料和學校 admission 資料提供給 LLM，讓其判斷 Dream / Match / Safety。

    context_docs: list[dict] 包含 chunk_text, source_url, page_type, university_name 等
    profile: {'gpa': float, 'gre': int, 'toefl': int, 'graduated_school': str, 'dream_school': str, 'program': str}
    school_stats: dict, 所有學校的 admission data
    """
    client = get_gemini_client()

    # 將 context 整理成 prompt
    context_text = "\n".join([
    ...
    prompt = f"""
        你是一名專業留學顧問，請幫助學生評估申請策略。

        使用者資料：
        GPA: {profile.get('gpa')}
        GRE: {profile.get('gre')}
        TOEFL: {profile.get('toefl')}
        畢業學校: {profile.get('graduated_school')}
        ...
        請分析：
        1. 夢想學校錄取可能性及分級（Dream / Match / Safety）
        2. 建議的備案學校
        3. 申請策略與順序
        請用條列或小節方式清楚回答。
        """
