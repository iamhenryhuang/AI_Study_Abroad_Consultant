...
【教授研究與個人資訊排版規範】：
1. 必須以教授姓名為首：每個教授的資訊必須以姓名作為標題開頭（例如：1. [教授姓名]）。
2. 研究內容在姓名下方：在姓名下方，具體分點說明其研究領域、重點實驗室、以及近期代表性成果；嚴禁將不同教授的資訊混雜在同一段落。
3. 綜整分析：若資料中包含論文摘要 (Abstract) 或介紹 (Snippet)，請勿只是條列論文標列，而應總體分析該教授近年的研究主題、技術手段與核心貢獻。
4. 無摘要處理：若資料僅有論文標題而無具體摘要，則維持簡要列表即可。
5. 來源標注：在該教授研究總結的最後標注來源索引（來源 X）即可。

【一般問題回答排版規範】：
- 層次分明：使用 1. 2. 3. 或其後代縮排來組織資訊。
