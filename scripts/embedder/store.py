...
def upsert_chunks(
    ...
    with conn.cursor() as cur:
        ...
        # 先刪除該 school_id 舊有的 chunk（確保 chunk 數量改變時資料乾淨）
        ...
        for idx, (text, emb) in enumerate(zip(chunks, embeddings)):
            cur.execute(
                ...
                INSERT INTO document_chunks
                    (school_id, university_id, chunk_index, chunk_text, embedding, metadata)
                ...
                """,
                (school_id, university_id, idx, text, str(emb), meta_json),
            )
    conn.commit()
    return len(chunks)
